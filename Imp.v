...
Proof.
  induction b; simpl; repeat rewrite optimize_0plus_sound; try reflexivity.
...
Proof.
  ...
  - intros H.
    induction H;
    try apply aeval_iff_aevalR in H1, H2;
    subst; reflexivity.
  - generalize dependent bv.
    induction b; intros; subst; constructor;
    try apply aeval_iff_aevalR; try apply IHb; try apply IHb1; try apply IHb2; reflexivity.
Qed.
(** [] *)
...
Fixpoint eval_fun_no_while (st : state) (c : com) : state :=
  match c with
    | <{ skip }> =>
        st
    | <{ x := a }> =>
        (x !-> (aeval st a) ; st)
    | <{ c1 ; c2 }> =>
        let st' := eval_fun_no_while st c1 in
        eval_fun_no_while st' c2
    | <{ if b then c1 else c2 end}> =>
        if (beval st b)
          then eval_fun_no_while st c1
          else eval_fun_no_while st c2
    | <{ while b do c end }> =>
        st  (* bogus *)
  end.
...
(** In a traditional functional programming language like OCaml or
    Haskell we could add the [while] case as follows:
        ...
        Fixpoint eval_fun (st : state) (c : com) : state :=
          match c with
            ...
            | while b do c end =>
                if (beval st b)
                  then eval_fun st (c ; while b do c end)
                  else st
          end.
    ...
    terminate: for example, the full version of the [eval_fun]
    ...
    of [eval_fun] cannot be written in Coq -- at least not without
    additional tricks and workarounds (see chapter [ImpevalFun]
    if you're curious about what those might be). *)

(* ================================================================= *)
(** ** Evaluation as a Relation *)

(** Here's a better way: define [eval] as a _relation_ rather than a
...
(** We'll use the notation [st =[ c ]=> st'] for the [eval] relation:
...
(** **** Exercise: 2 stars, standard (eval_example2)  *)
Example eval_example2:
  empty_st =[
    X := 0;
    Y := 1;
    Z := 2
...
(** **** Exercise: 3 stars, standard, optional (pup_to_n) 
    ...
    should update the state as shown in theorem [pup_to_2_eval],
...
Proof.
  ...
  - apply E_Ass; reflexivity.
  - apply E_WhileTrue with (X !-> 1 ; Y !-> 2 ; Y !-> 0 ; X !-> 2); try reflexivity.
    apply E_Seq with (Y !-> 2; Y !-> 0; X !-> 2); apply E_Ass; reflexivity.
    apply E_WhileTrue with (X !-> 0; Y !-> 3; X !-> 1; Y !-> 2; Y !-> 0; X !-> 2); try reflexivity.
...
(** Changing from a computational to a relational definition of
    ...
    In fact, this cannot happen: [eval] _is_ a partial function: *)

Theorem eval_deterministic: forall c st st1 st2,
     st =[ c ]=> st1  ->
     st =[ c ]=> st2 ->
     st1 = st2.
Proof.
...
Proof.
  ...
  (** Inverting [Heval] essentially forces Coq to expand one step of
      the [eval] computation -- in this case revealing that [st']
...
| N_Ass : forall a x, no_whilesR (CAss x a)
| N_Seq : forall c1 c2, no_whilesR c1 -> no_whilesR c2 -> no_whilesR (CSeq c1 c2)
| N_If : forall b c1 c2, no_whilesR c1 -> no_whilesR c2 -> no_whilesR (CIf b c1 c2)
.
...
Proof.
  ...
  - induction c; try constructor;
    inversion H as [H']; apply andb_prop in H' as [H1 H2]; apply IHc1 in H1; apply IHc2 in H2; assumption.
  - induction H; simpl; subst; try rewrite IHno_whilesR1, IHno_whilesR2; reflexivity.
Qed.
(** [] *)
...
Proof.
  intros c H. induction H; intro st.
  - exists st. constructor.
  - exists (x !-> (aeval st a); st). constructor; reflexivity.
  - destruct (IHno_whilesR1 st) as [s1 H1].
    destruct (IHno_whilesR2 s1) as [s2 H2].
    exists s2. apply (E_Seq c1 c2 st s1); assumption.
  - destruct (IHno_whilesR1 st) as [s1 H1].
    destruct (IHno_whilesR2 st) as [s2 H2].
    destruct (beval st b) eqn:Eb.
    + exists s1. apply E_IfTrue; try reflexivity; assumption.
    + exists s2. apply E_IfFalse; try reflexivity; assumption.
Qed.    
...
Fixpoint s_execute (st : state) (stack : list nat)
  ...
  | [],           s             => s
  | SPush n :: p, s             => s_execute st (n :: s) p
  | SLoad x :: p, s             => s_execute st ((aeval st (AId x)) :: s) p
  | SPlus   :: p, n1 :: n2 :: s => s_execute st (n2 + n1 :: s) p
  | SMinus  :: p, n1 :: n2 :: s => s_execute st (n2 - n1 :: s) p
  | SMult   :: p, n1 :: n2 :: s => s_execute st (n2 * n1 :: s) p
  | _       :: p, s             => s_execute st s p
  end.
...
Fixpoint s_compile (e : aexp) : list sinstr :=
  ...
  | ANum n => [SPush n]
  | AId x => [SLoad x]
  | <{ e1 + e2 }> => s_compile e1 ++ s_compile e2 ++ [SPlus]
  | <{ e1 - e2 }> => s_compile e1 ++ s_compile e2 ++ [SMinus]
  | <{ e1 * e2 }> => s_compile e1 ++ s_compile e2 ++ [SMult]
  end.
...
Proof.
  ...
  induction p1 as [| e p1 IH]; intros; try reflexivity.
  destruct e; repeat (destruct stack; try apply IH).
Qed.     
(** [] *)
...
Proof.
  intros st e.
  induction e; simpl; intros; repeat rewrite execute_app; try rewrite IHe1, IHe2; reflexivity.
Qed.
...
Proof.
  intros. apply (s_compile_correct_aux st e []).
Qed.  
...
Inductive eval : com -> state -> result -> state -> Prop :=
  | E_Skip : forall st,
  ...
  | E_IfFalse : forall st st' b c1 c2 s,
      ...
      st =[ c2 ]=> st' / s ->
      st =[ if b then c1 else c2 end]=> st' / s
  | E_SeqC : forall c1 c2 st st' st'' s,
      st  =[ c1 ]=> st' / SContinue ->
      st' =[ c2 ]=> st'' / s ->
      st  =[ c1 ; c2 ]=> st'' / s
  ...
  | E_WhileTrueC : forall st st' st'' b c s,
      beval st b = true ->
      st  =[ c ]=> st' / SContinue ->
      st' =[ while b do c end ]=> st'' / s ->
      st  =[ while b do c end ]=> st'' / SContinue
  | E_WhileTrueB : forall st st' b c,
      beval st b = true ->
      st =[ c ]=> st' / SBreak ->
      st =[ while b do c end ]=> st' / SContinue
  where "st '=[' c ']=>' st' '/' s" := (eval c st s st').

(** Now prove the following properties of your definition of [eval]: *)
...
Proof.
  ...
  - destruct (beval st b); discriminate.
  - apply IHeval2; trivial.
  - exists st. assumption.
Qed.
(** [] *)

(** **** Exercise: 4 stars, advanced, optional (eval_deterministic)  *)
Theorem eval_deterministic: forall (c:com) st st1 st2 s1 s2,
     st =[ c ]=> st1 / s1 ->
     st =[ c ]=> st2 / s2 ->
     st1 = st2 /\ s1 = s2.
Proof.
      ...
      generalize dependent st2.
      induction E1; intros st2 s2 E2; inversion E2; subst ; try (split; reflexivity).
      - apply IHE1. assumption.
      - rewrite H in H6. discriminate.
      - rewrite H in H6. discriminate.
      - apply IHE1. assumption.
      - apply IHE1_2. apply IHE1_1 in H1 as [H1 _]; subst; assumption.
      - inversion E2.
        apply IHE1_1 in H1 as [H1 _]; subst. apply IHE1_2. assumption.
        apply IHE1_1 in H2 as [_ contra]. discriminate.
      - apply IHE1 in H1 as [_ contra]. discriminate.
      - apply IHE1; assumption.     
      - rewrite H in H2. discriminate.
      - rewrite H in H2. discriminate.
      - rewrite H in H5. discriminate.
      - apply IHE1_1 in H3 as [H3 _]; subst. apply IHE1_2 in H7. destruct H7; subst. split; reflexivity.
      - destruct s;  apply IHE1_1 in H6 as [_ contra]; discriminate.
      - rewrite H in *; discriminate.
      - apply IHE1 in H3 as [_ contra]; discriminate.
      - apply IHE1 in H6 as [Hs _]. split; try reflexivity. assumption.
Qed.  
(** [] *)
End BreakImp.
...
(** **** Exercise: 4 stars, standard, optional (add_for_loop) 
    ...
    [eval] definition to define the semantics of [for] loops, and add
